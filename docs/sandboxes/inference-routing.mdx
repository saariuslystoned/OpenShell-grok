...
| Configuration | One workspace provider and model define the default `inference.local` backend. Personal subscription providers (`xai-grok-oauth` / `grok-subscription`) are an exception: the route is emitted only for an authenticated sandbox that has that exact provider attached. |
